...
type ResponseInputAssistantMessageWithPhase = OpenAI.Responses.EasyInputMessage & {
	role: 'assistant';
	phase?: string;
};
...
function rawContentToResponsesAssistantContent(part: Raw.ChatCompletionContentPart): OpenAI.Responses.ResponseInputContent | undefined {
	switch (part.type) {
		case Raw.ChatCompletionContentPartKind.Text:
			if (part.text.trim()) {
				return { type: 'input_text', text: part.text };
